 ...
 * @LastEditTime: 2022-04-29 16:02:45
...
const BlockBoard = ({ list, deleLines }: { list: BoardProps, deleLines?: number[] }) => {
    return (
        <TransitionGroup className='block' style={style}>
            {list.filter(({ x, y }) => { return y >= 0 }).map(({ x, y }, index) => {
                return <CSSTransition timeout={500} key={y * rol_x + x} classNames="block-css" onExit={(node:HTMLElement)=>{
                    if(deleLines?.indexOf(y)!=-1){
                        node.classList.add('block-css-done-exit')
                    }
                }}>
...
const NormalBoard = ({ styleBord, list, mvxPor, deleLines }: { styleBord: boardStyle, list: BoardProps, mvxPor?: MvxPor, deleLines?: number[] }) => {
    return (
        <>
            {styleBord === 'overlay' && (<OverlayBoard mvxPor={mvxPor as MvxPor}></OverlayBoard>)}
            {styleBord === 'block' && (<BlockBoard list={list} deleLines={deleLines}></BlockBoard>)}
